...
DEAnalysis <- function(scdata, id, path) {
    exprObj <- CreateSeuratObject(counts = as.matrix(scdata), project = "DE")
    Idents(exprObj) <- as.vector(id)
    print("Calculating differentially expressed genes:")
    for (i in unique(id)) {
        de_group <- FindMarkers(object = exprObj, ident.1 = i, ident.2 = NULL, only.pos = TRUE, test.use = "bimod")

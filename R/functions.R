...
#'
#' @param scdata single-cell RNA-seq count matrix
#' @param markers list of named data.frames with markers genes for each cluster
#' @param diff.cutoff minimum log2FC value
#' @param pval.cutoff minimum FDR value
#'
#' @return signature for to be used for \code{solveDampenedWLS}
#' @export
#'
buildSignatureMatrix <- function(scdata, markers, diff.cutoff = 0.5, pval.cutoff = 0.01) {
...
DEAnalysisMAST <- function(scdata, id, path) {
  ...
  for (i in unique(id)) {
    ...
    if (dim(DE)[1] > 1) {
      ...
      data_for_MIST <- as.data.frame(cbind(rep(rownames(counts), dim(counts)[2]), melt(counts), rep(groups, each = dim(counts)[1]), rep(1,
                                                                                                                                        dim(counts)[1] * dim(counts)[2])))
      colnames(data_for_MIST) = c("Gene", "Subject.ID", "Et", "Population", "Number.of.Cells")
      vbeta = data_for_MIST
      vbeta.fa <- FromFlatDF(vbeta, idvars = c("Subject.ID"), primerid = "Gene", measurement = "Et", ncells = "Number.of.Cells", geneid = "Gene",
                             cellvars = c("Number.of.Cells", "Population"), phenovars = c("Population"), id = "vbeta all")
      vbeta.1 <- subset(vbeta.fa, Number.of.Cells == 1)
      # .3 MAST head(colData(vbeta.1)) ??
      ...
      colnames(lrTest.table) <- c("Gene", "test.type", "p_value", paste("log2.mean.", "Cluster_Other", sep = ""), paste("log2.mean.", i,
                                                                                                                        sep = ""), "log2fold_change", "Auc")
...
buildSignatureMatrixMAST <- function(scdata, id, path, diff.cutoff = 0.5, pval.cutoff = 0.01) {
  ...
  # for each cell type, choose genes in which FDR adjusted p-value is less than 0.01 and the estimated fold-change is greater than 0.5
  numberofGenes <- c()
  for (i in unique(id)) {
  ...
  # need to reduce number of genes for each subset, order significant genes by decreasing fold change, choose between 50 and 200 genes for
  # each, iterate and choose matrix with lowest condition number
  conditionNumbers <- c()
  for (G in 50:200) {
    Genes <- c()
    j = 1
